...
export async function requestValuationContactAction(input: {
  name: string;
  email: string;
  /** Optional callback number; the reply itself goes by email. */
  phone?: string;
  ...
  // Deliberately permissive, like every other address check in this repo:
  // real addresses vary more than any regex allows, and the column is what
  // the follow-up is sent to, not an identity claim.
